...
describe("processAction", () => {
  it("approves actions without policy constraints", () => {
    const result = processAction({ type: "SUBMIT_ADVISORY", agentId: "a1", payload: {} });
    expect(result.result).toBe(PipelineResult.OK);
    expect(result.reason).toBeNull();
  });

  it("blocks actions that violate policy", () => {
  ...
  it("logs OVERRIDE_ATTEMPTED to ledger when policy blocks", () => {
    ...
    expect(result.ledger[0].payload.agent_id).toBe("a1");
  });
});
...
describe("processBatch", () => {
  it("processes multiple actions in sequence", () => {
    const actions = [
      { sequence: 0, type: "SUBMIT_ADVISORY", agentId: "a1", payload: {} },
      { sequence: 1, type: "HEARTBEAT", agentId: "a2", payload: {} },
    ];
  ...
  it("accumulates ledger across batch", () => {
    const { policy } = compilePolicy({ waiver: { rationale_min_length: 50 } });
    const actions = [
      {
        sequence: 0,
